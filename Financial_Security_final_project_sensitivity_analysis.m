...
M = [1/12,2/12,3/12,6/12,1]; %Maturity for zero-coupon interest rate
s = [0.02 0.01 0.02 0.04 0.05];%interest rate corresponding to each maturity
s = s./100;
...
M1 = [1/50,1/12,2/12,3/12,6/12,1]; %Maturity for Libor interest
s1 = [0.0007150,0.0009813,0.0013250,0.0016750,0.0019250,0.0026700];%Libor rate corresponding to each maturity
...
sigma_r = 0.0005; %volatility for short rate
sigma_r_list = [1:10];
sigma_r_list = sigma_r_list.*0.005; %initialize the sensitivity variable
...
r(1) = 0.0006338; %the initial short rate estimate (using overnight Libor rate)
ytm1 = 0.32; %Yield to maturity with Maturity=3
ytm2 = 0.28; %Yield to maturity with Maturyti = T-Delta with interpolation
k = 1; %parameter in the Equity portion of max function 
k_list = [0:10];
k1 = 1;%parameter in the Libor portion of max function
Num = 10000; %Number of paths
...
%variables to store the data for each step
total_equity_record = zeros(Num,T*workday+1);
total_r_record = zeros(Num,T*workday+1);
total_L_TdT_record = zeros(Num,1);
total_L_record = zeros(Num,1);
total_payoff_record = zeros(Num,1);
price = zeros(length(sigma_r_list),1);
for h = 1:length(sigma_r_list)
    for j=1:Num
        ...
        for i= 1:workday*T
            ...
            dr = (theta-a*r(i))*dt+sigma_r_list(h)*sqrt(dt)*e_r(i); %calculate dr =(\theta(t)-ar)dt+sigma_r\sqrt(dt)e_r)
        ...
        total_equity_record(j:j,:) = equity;
        total_r_record(j:j,:) = r;
        ...
        L_TDT = (1-p_TdT)/(delta*p_TdT); %Libor rate calculation
        total_L_TdT_record(j) = L_TDT; %record the L(T-Delta,T-Delta,T)
        L_0T = (p_0T-p_0Td)/(delta*p_0Td); %get L(0,T-Delta,T)
        total_L_record(j) = L_TDT/L_0T; %Calculate Libor ratio
        payoff = max(0,(equity(T*workday+1)/S0-k)*(k1-L_TDT/L_0T)); %Payoff
        total_payoff_record(j) = payoff; %record payoff
        discount = exp(-(sum(r)-r(1))*dt); %discount factor
        res = res+payoff*discount; %sum up all the payoff*discount factor for expectation
    end
    price(h) = res/Num; %res/Num is the final expectation that is the answer to the question.
end

%plot the short rate curve
% z = [1:757];
% z = z./workday;
% figure
% for i=1:Num
%     plot(z,total_r_record(i:i,:));
%     hold on;
% end
% 
% title('Calibrated Short rate Curve');
% xlabel('Time (years)');
% ylabel('rate');